...
export class AuthserviceService {
  ...
  login(user: User): Observable<Response> {
    return this.http.post<Response>(this.loginEndpoint, user).pipe(
      map((res: Response) => {
        this.saveLocalStorage(
          res.data.token || "",
          res.data.accountingFirmId,
          res.data.userName,
          res.data.lastName,
          res.data.name,
          res.data.id
  ...
  private saveLocalStorage(
    ...
    localStorage.setItem(
      environment.localStorage.userAccountingFirm,
      userAccountingFirm
    );
    ...
    localStorage.setItem(environment.localStorage.userId, userId);
  }
